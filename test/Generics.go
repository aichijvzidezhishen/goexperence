...
const (
	...
	if ok then
		redis.call("hmset", limit_key, "token", new_tokens, "last_time", now, "last_event", time_act)
	end
	...
	redis.call("hmset", limit_key, "token", new_tokens, "last_time", now)
	if last_event == event_act and qps > 0 then
		pre_event = time_act - math.ceil(new_tokens / qps)
		if pre_event >= now then
			redis.call("hmset", limit_key, "last_event", pre_event)
		end
	end
	return true
	`
