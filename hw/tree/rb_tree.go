...
func New() *Rbtree {
	node := &RedNode{nil, nil, nil, BLACK, nil}
	return &Rbtree{
		NIL:   node,
		root:  node,
		count: 0,
...
// Left Rotate
func (rbt *Rbtree) LeftRotate(no *RedNode) {
...
// Right Rotate
func (rbt *Rbtree) RightRotate(no *RedNode) {
	if no.Left == rbt.NIL {
		return
	}
...
func (rbt *Rbtree) Insert(no *RedNode) {
	x := rbt.root
	var y *RedNode = rbt.NIL
...
func (rbt *Rbtree) insertFixup(no *RedNode) {
...
func LeftRotateTest() {
	...
	x := &RedNode{rbtree.NIL, rbtree.NIL, rbtree.NIL, BLACK, i10}
	rbtree.root = x
	y := &RedNode{rbtree.root.Right, rbtree.NIL, rbtree.NIL, RED, i12}
	rbtree.root.Right = y
