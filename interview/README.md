...
9. 数据备份和恢复
    mongodump
    mongorestore
    mongodump --db test --collection myCollection --out /data/backup
    mongorestore --db test --collection myCollection /data/backup/test/myCollection.bson
10. 数据库监控
...
8.  过期策略
    ...
    b. 被动删除  + 主动删除 
        1. 被动删除（惰性删除） 
        触发条件：当客户端访问某个键时，Redis 会先检查该键是否过期。若过期，则删除该键并返回 “键不存在”；若未过期，则正常处理请求。
        优点：仅在键被访问时才执行删除操作，不主动消耗 CPU 资源，对性能影响最小。
        缺点：若过期键长时间未被访问，会一直占用内存，可能导致内存泄漏。
        2. 主动删除（定期删除）
        触发机制：Redis 会周期性地执行过期键检查任务，主动删除部分过期键。具体规则如下：
        执行频率：默认每秒执行 10 次（可通过 hz 配置项调整，范围 1-500）。
        检查逻辑：
        每次从过期字典中随机选取 20 个键，检查是否过期。
        删除其中过期的键。
        若过期键比例超过 25%（即 20 个中超过 5 个过期），则重复步骤 1-2，直到过期键比例低于 25% 或检查次数达到上限（避免长时间占用 CPU）。
        优点：主动清理过期键，减少内存浪费，避免大量过期键堆积。
        缺点：检查频率和样本量需平衡性能与内存占用：
        频率过高或样本量过大，会消耗过多 CPU 资源
        频率过低或样本量过小，过期键清理不及时，内存占用过高。
...
10. redis 优化
    ...
    1. 处理big key
    2. hot key（热key）
11. 精通redis集群原理支持（一致性hash 算法） 主从复的撒的制 哨兵模式，cluster
12. 消息队列 krafa、redis 、nsp（自己实现下）
...
5. 说说HTTP的状态码，301和302的区别？
   301：永久重定向，浏览器会缓存这个重定向，下次直接访问新的地址
   302：临时重定向，浏览器不会缓存这个重定向，每次访问都会去检查重定向地址
6. 对称加密和非对称加密  
7. dns解析过程
8. csrf 攻击 如何避免
...
19. TCP 和 UDP 分别对应的常见应用层协议
    TCP：HTTP/HTTPS、SMTP/POP3/IMAP、FTP 
    UDP：DNS、DHCP、RTP、SNMP
20. 保活计时器
...
26. TCP四次挥手过程
    断开链接
27. TCP的粘包和拆包
    纯裸的TCP协议是没有粘包和拆包的问题的，因为TCP是流式协议，发送方和接收方都会有一个缓冲区，发送方会根据缓冲区的情况来将数据发送出去，接收方会根据缓冲区的情况来接收数据。
    加入消息头
...
33. TCP报文首部有哪些字段，作用
    ...
    8. 16位紧急指针：一个正的偏移量。它和序号字段的值相加表示最后一个紧急数据的下一字节的序号。因此，确切地说，这个字段是紧急指针相对当前序号的偏移，不妨称之为紧急偏移。TCP的紧急指针是发送端向接收端发送紧急数据的方 法。
...
3. 进程有哪些状态
   就绪、运行、阻塞、创建、终止
4. 僵尸进程 ： 子进程已经终止，但是父进程没有仍在运行，没有调用wait（） 等系统调用获取子进程的信息，释放子进程占用的资源，导致子进程的PCB（进程控制块）一直存在，占用系统资源，这种情况下，子进程就是僵尸进程。
5. 孤儿进程 ： 一个进程的父进程已经终止或者不存在，但是该进程仍在运行，这种情况下，该进程就是孤儿进程。
6. 进程有哪些调度算法·
7. 进程间通信有哪些方式
    - 管道/匿名管道
    - 信号
...
### 二叉树
1. 基本性质
   - ![详细](https://oss.javaguide.cn/github/javaguide/cs-basics/data-structure/%E4%BA%8C%E5%8F%89%E6%A0%91-2.png)
   - 高度
   - 层数 二叉树 的第 i 层至多拥有 2^(i-1) 个节点
   - 深度 2^k -1
2. 完全二叉树
   - 当根节点的值为 1 的情况下，若父结点的序号是 i，那么左子节点的序号就是 2i，右子节点的序号是 2i+1
3. 满二叉树  
    - 二叉树的层数为 K，且结点总数是(2^k) -1 
...
5. 二叉查找树（BST）
   ...
   -  左右子树均为二叉查找树
6. 平衡二叉树
   - 基本概念：采用旋转操作保持平衡，LL 旋转、RR 旋转、LR 旋转和 RL 旋转。其中 LL 旋转和 RR 旋转分别用于处理左左和右右失衡，而 LR 旋转和 RL 旋转则用于处理左右和右左失衡。
   - 
7. 红黑树
   - 
   - 二叉平衡树解决二叉树不平衡导致的性能问题，通过调整树的结构，来达到高度的平衡状态，但是频繁的调整树的结构也是一种开销，而红黑树就是舍去一部分平衡性来获得低的调整开销，但是红黑树也存在‘右倾现象’，查找性能又会降低。
8. 
### 栈
### 队列
    看下lru 结合redis
...
7. 递归复制系统目录
    cp -r source_directory distination_directory
...
9. 删除
     ...
     rm directory_name
10. 文件查找
    find - 按文件名、类型、大小等查找文件
    find . -name "*.txt" 在当前目录查找所有txt文件
    find . -type f 查找所有文件
