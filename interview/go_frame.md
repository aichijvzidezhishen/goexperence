...
答：Go 传参和返回值是通过 FP+off在调用函数的栈帧中set 实现，并且存储。FP 栈底寄存器，指向一个函数栈的顶部;PC 程序计数器，指向下一条执行指令;SB 指向静态数据的基指针，全局符号;SP 栈顶寄存器。
...
核心：直接背诵，面试可直接应答，适配西安15K Go后端面试
- 1. GMP模型：G是Go语言的轻量级协程（用户态，开销极小），M是操作系统的物理线程（内核态），P是处理器（用于调度G和M的绑定）；核心逻辑是每个P会绑定一个M，P内部维护本地G队列，同时还有全局G队列，通过work stealing（工作窃取）机制，让空闲的P去窃取其他P的G来执行，实现负载平衡，提高并发效率。

- 2. goroutine泄漏：主要有4种常见场景，一是channel只发不收或只收不发，导致goroutine一直阻塞等待；二是goroutine内部存在死循环，无法正常退出；三是goroutine持有锁后未释放，一直阻塞在锁竞争上；四是context未正确传递或未调用取消方法，导致goroutine无法收到退出信号，一直处于运行状态。

- 3. channel安全：channel底层是hchan结构体，该结构体内部包含一个互斥锁（mutex），所有对channel的操作（发送、接收、关闭）都会先获取这个互斥锁，操作完成后释放锁，保证了同一时间只有一个goroutine能操作channel，因此channel是并发安全的。

- 4. 有缓冲vs无缓冲：两者核心区别是是否有缓冲空间、是否同步阻塞。无缓冲channel没有缓冲空间，发送操作（send）和接收操作（recv）必须同时就绪，一方先执行会阻塞，直到另一方就绪，属于同步通信；有缓冲channel有固定的缓冲容量，发送操作只有在缓冲区满时才会阻塞，接收操作只有在缓冲区空时才会阻塞，属于异步通信，可实现解耦。

- 5. Mutex/Channel选型：两者都是Go中实现并发控制的方式，选型核心看使用场景。如果是需要保护共享变量、控制共享资源的访问（比如多个goroutine操作同一个变量），优先用Mutex（互斥锁）；如果是需要实现goroutine之间的数据传递、任务分发（比如一个goroutine给另一个goroutine传数据），优先用channel，更贴合Go的并发设计理念。

- 6. context作用：主要有两个核心作用，一是控制goroutine的生命周期，通过context的取消（Cancel）、超时（Timeout）方法，让子goroutine收到退出信号，避免goroutine泄漏；二是传递请求链路中的公共信息（比如请求ID、日志上下文、trace信息），让整个请求链路中的goroutine都能获取到这些信息，方便排查问题和追踪请求。

- 7. 内存泄漏：指程序中已分配的内存无法被回收，导致内存占用越来越多，最终可能引发内存溢出。常见场景有3种，一是goroutine泄漏（最常见），二是长生命周期的对象持有短生命周期对象的引用（比如全局变量持有临时变量的指针），导致临时变量无法被GC回收；三是切片、map只增不减，不断向其中添加元素但不删除，导致内存一直被占用。

- 8. make/new区别：两者都是Go中用于内存分配的内置函数，但适用场景和返回值完全不同。make主要用于分配slice、map、channel这三种引用类型，返回的是引用类型本身（不是指针），并且会初始化其内部结构（比如slice初始化底层数组，map初始化哈希表）；new用于分配任意类型的内存，返回的是该类型的指针，并且只会将内存初始化为零值，不会初始化内部结构（比如new(map)返回的是*map，该map还是nil）。

- 9. 垃圾回收（GC）：Go的垃圾回收采用三色标记法，核心流程是先将所有对象标记为白色（未标记），然后从根对象（比如全局变量、goroutine栈上的变量）开始，将可达对象标记为灰色，再将灰色对象的引用对象标记为灰色，直到没有灰色对象，最后将白色对象（不可达对象）回收。同时采用并发GC机制，GC执行时不会完全阻塞所有goroutine，只会在特定阶段有短暂的STW（停止世界），减少对程序运行的影响。

- 10. 逃逸分析：是Go编译器的一种优化机制，用于判断变量的内存分配位置（栈或堆）。编译器会在编译时分析变量的作用域和引用关系，如果变量的引用不会传出当前函数（比如只在函数内部使用，不返回指针、不传入channel、不被闭包引用），就会分配在栈上（栈内存自动回收，效率高）；如果变量的引用会传出当前函数（比如返回指针、传入channel、被闭包引用），就会逃逸到堆上（堆内存由GC回收，效率略低）。

- 11. 协程（goroutine）和线程（thread）的区别：核心是轻量性和调度方式不同。goroutine是用户态，由Go runtime调度，开销极小（每个goroutine初始栈大小仅2KB，可动态扩容），一台机器可同时运行数万甚至数十万goroutine；线程是内核态，由操作系统调度，开销大（初始栈大小通常为1MB），可运行的数量有限。此外，goroutine的调度由P和M配合完成，无需操作系统干预，效率远高于线程切换。

- 12. Go的defer关键字作用及执行顺序：defer用于延迟执行函数，通常用于资源释放（如关闭文件、释放锁、关闭连接），确保即使函数发生异常，延迟函数也能执行。执行顺序遵循“后进先出（LIFO）”原则，即多个defer语句，后定义的先执行；同时defer函数会在函数return之前执行，且能读取到函数内的变量（包括返回值）。

- 13. 什么是原子操作（sync/atomic包）：原子操作是不可中断的操作，用于解决并发场景下共享变量的安全问题，比互斥锁（Mutex）更高效（无需上下文切换）。sync/atomic包提供了常见的原子操作，比如增减（Add）、加载（Load）、存储（Store）、交换（Swap）等，适用于简单的共享变量操作（如计数器），复杂场景仍需用Mutex。

- 14. Go的接口（interface）是什么，有什么特点：接口是一种抽象类型，定义了一组方法签名（只声明方法，不实现），任何类型只要实现了接口的所有方法，就隐式实现了该接口（无需显式声明）。Go的接口是“鸭子类型”，即“长得像鸭子，就是鸭子”，核心特点是解耦，让代码更灵活、可扩展，比如实现多态、依赖注入。

- 15. 切片（slice）和数组（array）的区别：数组是固定长度的，声明时必须指定长度（如var arr [5]int），长度不可修改，赋值和传参时会拷贝整个数组，效率较低；切片是动态长度的，基于数组实现（底层是数组），声明时无需指定长度（如var s []int），长度和容量可动态扩容，赋值和传参时只拷贝切片头（包含指针、长度、容量），效率较高，日常开发中优先使用切片。

- 16. 切片的扩容机制：当切片的长度（len）等于容量（cap）时，再向切片添加元素会触发扩容。扩容规则：当原容量小于1024时，扩容后的容量是原容量的2倍；当原容量大于等于1024时，扩容后的容量是原容量的1.5倍；此外，扩容时会创建一个新的底层数组，将原切片的元素拷贝到新数组，再返回新切片，原切片的底层数组会被GC回收。

- Go的error和panic的区别及使用场景：error用于表示可预期的、轻微的错误（如参数错误、文件打开失败），是可恢复的，通常通过返回值传递，程序员可根据error做相应处理；panic用于表示不可预期的、严重的错误（如数组越界、空指针引用），会中断当前goroutine的执行，触发defer函数，之后程序会崩溃（除非用recover捕获），日常开发中应避免主动使用panic，仅在遇到致命错误时使用。

- recover的作用及使用注意事项：recover用于捕获panic，避免程序崩溃，只能在defer函数中使用。当goroutine发生panic时，会依次执行该goroutine中的defer函数，若defer函数中调用了recover，就能捕获到panic信息，让程序恢复正常执行；若未捕获，程序会崩溃。注意：recover只能捕获当前goroutine的panic，无法捕获其他goroutine的panic。

- Go的并发控制方式有哪些：主要有4种，一是channel（用于数据传递和同步）；二是Mutex（互斥锁，保护共享资源）；三是RWMutex（读写锁，读多写少场景下比Mutex更高效，多个读 goroutine可同时访问，写goroutine会独占）；四是sync.WaitGroup（用于等待一组goroutine执行完成，比如批量启动goroutine，等待所有goroutine执行完毕后再继续）。